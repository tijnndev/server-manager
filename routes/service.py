...
"""
        ...
        os.system('/etc/server-manager/venv/docker-compose up -d')
...
def start_service(name):
    ...
    try:
        ...
        subprocess.run(['/etc/server-manager/venv/docker-compose', 'up', '-d'], check=True)
...
def stop_service(name):
    ...
    try:
        os.chdir(os.path.join(ACTIVE_SERVERS_DIR, name))
        os.system('/etc/server-manager/venv/docker-compose down')
...
def get_uptime(service_name):
    ...
    try:
        ...
        result = subprocess.run(['/etc/server-manager/venv/docker-compose', 'ps', '-q'], capture_output=True, text=True, check=True)
...
def stream_logs(name):
    ...
    try:
        ...
        logs_command = ['/etc/server-manager/venv/docker-compose', 'logs', '-f', '--tail', '50']
...
def rebuild(name):
    ...
    try:
        os.chdir(os.path.join(ACTIVE_SERVERS_DIR, name))
        os.system('/etc/server-manager/venv/docker-compose build')
